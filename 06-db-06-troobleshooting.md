...
Проблема, скорее всего, вызвана активным механизмом истечения времени жизни ключей. Этот механизм запускается каждые 100мс и по умолчанию истекают 20 ключей за цикл (ACTIVE_EXPIRE_CYCLE_LOOKUPS_PER_LOOP), а при обнаружении количества записей больше 25% может зацикливаться и блокировать базу пока количество не станет меньше 25%.
